import { LoggingsController, LoggingsLevels, LoggingsMessage } from "./types";
import defaults, { LoggingsStatus } from "./defaults";
...
export function Controller(isolated: LoggingsController, args: LoggingsMessage[]) {
    ...
    if (options.console && options.current_level >= options.level) {
        ...
        if (message_csl.includes("{title}")) {
            message_csl = message_csl.replaceAll("{title}", chalk[options.controller_color](options.controller_title));
        }
        if (message_csl.includes("{status}")) {
            message_csl = message_csl.replaceAll("{status}", chalk[LoggingsStatus[options.current_level]](options.current_level));
        }
        if (message_csl.includes("{message}")) {
            message_csl = message_csl.replaceAll("{message}", Formatter(args).message_csl);
        }
        console[LoggingsLevels[options.current_level]](message_csl);
    }
    /**
