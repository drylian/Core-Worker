...
export function InstallerSetupThree() {
  ...
  <div class="max-w-md bg-slate-700 p-4 m-2 rounded-md shadow-md text-center justify-center">
    ...
    <p class="text-gray-300 mb-4">${i18n.t("installer.CreateAccountMessage")}.</p>
    <form action="/install/setup/one" method="post" class="text-left">
      ...
      <input type="text" id="title" required name="title" placeholder="${i18n.t("installer.AccountName")}" class="w-full px-3 py-2 mb-3 bg-gray-600 rounded-md text-white">

      <label for="url" class="block text-gray-400 text-sm mb-2">${i18n.t("installer.PanelUrl")}</label>
      <input type="text" id="url" required name="url" placeholder="${i18n.t("installer.email")}" class="w-full px-3 py-2 mb-3 bg-gray-600 rounded-md text-white">

      <label for="port" class="block text-gray-400 text-sm mb-2">${i18n.t("installer.PanelPort")}</label>
      <input type="number" id="port" required name="port" placeholder="${i18n.t("installer.AccountName")}" class="w-full px-3 py-2 mb-4 bg-gray-600 rounded-md text-white">
