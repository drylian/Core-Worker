...
async function configureProductionMode() {
    const configs = [
        { path: "./builded/src/config/settings.json", data: { mode: "production" } },
    ]
    for (const config of configs) {
        ...
        if (clear) {
            if (existingData[clear] !== undefined) {
                delete existingData[clear];
                const updatedContent = JSON.stringify(existingData, null, 2);
                await fss.writeFile(path, updatedContent, 'utf-8');
                console.log(`Propriedade "${clear}" do ${path} foi limpada.`);
            } else {
                console.log(`Propriedade "${clear}" do ${path} não foi encontrada.`);
            }
        } else {
